...
function print_miniplan( $atts ) {
    ...
    if ($results === null || $results == "" || empty($results)) {
        if (WP_DEBUG === true) {
            error_log("Something went wrong, while fetching the miniplan!");
        }
...
function print_miniplan_notification( $atts ) {
    ...
    $mpl_data = $wpdb->get_results(
        'SELECT beginning,notification FROM `' . $table_name . '` WHERE feed_id = \'' . intval($atts["id"]) . '\' ORDER BY beginning DESC', OBJECT
    ...
    if ( $latest == NULL ) {
        if (WP_DEBUG === true) {
            error_log("Something went wrong, while fetching the miniplan notifications!");
        }
...
--->
    ...
    $admin_action = get_query_var( "miniplan_admin_action", "" );
        ...
        case "upload":
            if ( $submitState ) {
                $error_msg = miniplan_add_new($feed_id, get_query_var( "mpl_text", "" ), get_query_var( "mpl_attendance", "" ), get_query_var( "mpl_notification", "" ), get_query_var( "mpl_beginning", "" ), get_query_var( "mpl_until", "" ));
                if ( $error_msg === null ) {
                    ...
                    $content .= miniplan_message( $error_msg, "error" );
                }
        ...
        case "edit":
            if ( $submitState ) {
                $error_msg = miniplan_edit_existing(
                    $master_mpl->id,
                    $feed_id,
                    get_query_var( "mpl_text", "" ),
                    get_query_var( "mpl_attendance", "" ),
                    get_query_var( "mpl_notification", "" ),
                    get_query_var( "mpl_beginning", "" ),
                    get_query_var( "mpl_until", "" )
                );
                if ( $error_msg === null ) {
                    ...
                    if ( get_query_var( "mpl_send_mail", "false" ) === 'TRUE' ) {
                        ...
                        $content .= miniplan_send_mail_notification(
                            get_query_var( 'mpl_email_address' ),
                            $subject,
                            $message
                        );
                    }
        ...
        case "delete":
            if ( $submitState ) {
                if ( $master_mpl->id != "-1" || $submitState == "FORCE" ) {
                    $error_msg = miniplan_delete_existing( $master_mpl->id );
                    if ( $error_msg === null ) {
        ...
        default:
            $content .= '<p>Wähle eine Aktion:</p><form action="' . $_SERVER['REQUEST_URI'] . '#miniplan_admin_panel" method="get">
                ...
                <button name="miniplan_admin_action" id="upload_btn" class="btn btn-default upload" value="upload">Einen neuen Plan erstellen</button>
...
function miniplan_date_format( $strdate , $mode="human" ) {
    ...
    if ( $dtdate instanceof DateTime ) {
        ...
        if (WP_DEBUG === true) {
            error_log("Got a wrong date format. (in miniplan_date_format)");
        }
